...
import Aadhunik from '../assets/fonts/Aadhunik.ttf'
...
const splitText = (text, columns) => {
  ...
  for (const char of text) {
    ...
    charInRow += 1
    if (
      (charInRow >= charLimit && isWhiteSpace(char)) ||
...
export default class SVGText extends React.Component {
  componentWillMount() {
    return new Promise((resolve, reject) =>
      TextToSVG.load(Aadhunik).then((err, textToSVG) => {
        ...
        this.setState(
          {
            paths: rows.map((row, index) => {
              options.y =
                index > 0 ? options.y + options.fontSize * 2 : options.y
          ...
          >
            {this.state.paths.map((path, index) => (
              <path
                ...
                id={`${this.props.id}path${index}`}
                key={`${this.props.id}path${index}`}
              />
            ))}
          </svg>
        ))
